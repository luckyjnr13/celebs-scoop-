...
# -----------------------------------------------------------------------------
# Background Audio Link (Direct audio streaming URL)
AUDIO_URL = "https://audio.com/starstream/audio/jennie-blackpink-like-jennie/download"

# Hidden/Discrete HTML Audio Player auto-looping in the layout background
st.markdown(f"""
...
# User-facing playback bar for browsers requiring direct interaction context
...
def fetch_celebrity_data(query):
    """Fetches real-time profile pictures, accurate bios, and metadata."""
    if not query.strip():
        return None
...
def generate_quiz_question(celeb_name, bio_text):
    """Dynamically parses details from Wikipedia to form safe, unique questions."""
    ...
    if len(facts) > 1:
        ...
        # Redact the name to turn it into a trivia question
    ...
    # Generic fallback dynamic variant
...
# -----------------------------------------------------------------------------
# 5. STATE ENGINE (PREVENTS REPETITIVE QUIZ CYCLES)
# -----------------------------------------------------------------------------
...
if search_input and search_input != st.session_state.current_celeb:
    ...
    if profile:
        ...
        # Generate entirely new quiz structure unique to session instance history
...
# Display Profile & Quiz inside a single cohesive layout view
if st.session_state.celeb_profile:
    ...
    with col1:
        st.image(prof["image"], use_container_width=True, caption=prof["description"])
    ...
    if st.session_state.quiz_data:
        ...
        # Unique key identifier dynamically generated based on tracking stats to clear stale selections
        ...
        if st.button("Submit Verified Answer"):
            ...
            # Log structural identifiers to ensure it never repeats across future iterations
            st.session_state.asked_questions.add(f"{prof['name']}_{qz['question'][:15]}")
            
            # Instantly generate a refreshed, non-repeating configuration question
