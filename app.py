...
# 1. FORCE the sidebar to be open automatically on all devices
st.set_page_config(
    page_title="Celebs Scoop",
    page_icon="✨",
    layout="centered",
    initial_sidebar_state="expanded"  # Locks sidebar open on screen load
)

# 2. Shared Styling for Glamorous Hollywood Theme
st.markdown("""
    <style>
    .stApp {
...
# 4. Crash-Proof Quick Navigation Links
st.write("---")
st.markdown("### 🚀 Quick Navigation Links:")

try:
    st.page_link("pages/1_🔍_Search.py", label="Open Celebrity Search", icon="🔍")
    st.page_link("pages/2_🧠_Quiz.py", label="Take the Unlimited Quiz", icon="🧠")
except Exception:
    st.info("👈 Use the open sidebar menu on the left side of your screen to change pages!")
